...
build_kernel() {
    ...
    local LLVM_FLAG=""
    local REALMODE_CFLAGS_VAR="-std=gnu11 -Wno-error"
    if [ "$ARCH" = "arm64" ] && [[ "$CROSS_COMPILE" == *"musl"* ]]; then
        LLVM_FLAG="LLVM=1 LLVM_IAS=1"
        # For ARM64 with LLVM, add -Wa,-m16 for 16-bit mode support
        REALMODE_CFLAGS_VAR="-std=gnu11 -Wno-error -Wa,-m16"
        log_info "Using LLVM toolchain for ARM64 cross-compilation"
    else
        # For x86_64 native builds with GCC, use basic flags only
        # The -Wa,-m16 flag doesn't work with GNU binutils assembler
        REALMODE_CFLAGS_VAR="-std=gnu11 -Wno-error"
    fi

    set +e
    stdbuf -oL -eL make -j"$JOBS" ARCH="$KERNEL_ARCH" CROSS_COMPILE="$CROSS_COMPILE" $LLVM_FLAG KCFLAGS="-std=gnu11 -Wno-error" HOSTCFLAGS="-std=gnu11 -Wno-error" REALMODE_CFLAGS="$REALMODE_CFLAGS_VAR" "$MAKE_TARGET" 2>&1 | \
