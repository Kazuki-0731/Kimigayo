...
apply_security_hardening() {
    ...
    if [ "$ARCH" = "arm64" ]; then
        make ARCH="$KERNEL_ARCH" LLVM=1 KCFLAGS="-std=gnu11" olddefconfig > /dev/null 2>&1 || true
    else
        make ARCH="$KERNEL_ARCH" KCFLAGS="-std=gnu11" olddefconfig > /dev/null 2>&1 || true
    fi
...
build_kernel() {
    ...
    set +e
    stdbuf -oL -eL make -j"$JOBS" ARCH="$KERNEL_ARCH" CROSS_COMPILE="$CROSS_COMPILE" $LLVM_FLAG KCFLAGS="-std=gnu11" "$MAKE_TARGET" 2>&1 | \
